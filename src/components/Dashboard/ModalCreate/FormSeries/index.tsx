...
import { Cake, Calendar, File, PlusCircle, StickyNote, User } from "lucide-react";

import { TagType } from "..";
import { createSeriesSchema } from "../../../../schemas/seriesSchema";
...
type SeriesType = z.infer<typeof createSeriesSchema>;

interface Props {
  type: "tvShow" | "soapOpera" | "anime";
  setShowModal: (visible: boolean) => void;
  tags: TagType[];
}

function FormSeries({ type, setShowModal, tags }: Props) {
  const [image, setImage] = useState<string | ArrayBuffer | null>(null);

  const { register, handleSubmit } = useForm<SeriesType>({
  ...
  const onSubmit: SubmitHandler<SeriesType> = async (data: SeriesType) => {
    try {
      const {
        title,
        description,
      ...
      if (type === "tvShow") {
        const { status } = await seriesService.createTvShow(params);

        if (status === 201) {
          alert("Tvshow created with success");
          setShowModal(false);
      ...
      if (type === "soapOpera") {
        const { status } = await seriesService.createSoapOpera(params);

        if (status === 201) {
          alert("Soap opera created with success");
          setShowModal(false);
          window.location.reload();
        }
      }
      ...
      if (type === "anime") {
        const { status } = await seriesService.createAnime(params);

        if (status === 201) {
          alert("Anime created with success");
          setShowModal(false);
          window.location.reload();
        }
      }
